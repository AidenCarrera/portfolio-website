...
export default function CassetteDeck({ activeSnippet }: CassetteDeckProps) {
  ...
  // Track previous snippet ID to reset state during render (avoids useEffect setState lint)
  const [prevSnippetId, setPrevSnippetId] = useState(activeSnippet?.id);

  // Reset state during render if snippet changes
  ...
  // Handle side effects (Audio element & Animation) when snippet changes
  useEffect(() => {
  ...
  const handleSeek = useCallback(
    (e: React.MouseEvent | MouseEvent, commit: boolean = false) => {
      const newTime = calculateTime(e);

      // Always update visual time
      setCurrentTime(newTime);

      // Only update audio if committing (mouse up) or if clicked directly (not dragging yet)
  ...
  useEffect(() => {
    const handleTimeMouseMove = (e: MouseEvent) => {
      ...
      handleSeek(e, false); // Visual only
    };

    const handleTimeMouseUp = (e: MouseEvent) => {
      if (!isDraggingTime) return;
      setIsDraggingTime(false);
      handleSeek(e, true); // Commit seek
    };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
            ...
            <div className="bg-slate-800 rounded-lg p-4 border border-slate-600 shadow-inner font-mono relative overflow-hidden">
              <div className="flex justify-between items-center text-brand mb-2">
                <div className="text-xs text-brand tracking-widest uppercase">
                  Playback
                </div>
              ...
              <div
                ref={progressBarRef}
                className={`w-full bg-slate-800 h-2 rounded-full overflow-visible relative group ${
                  activeSnippet
                    ? "cursor-pointer hover:h-2 transition-height"
                    : ""
                }`}
              ...
              >
                ...
                {/* Scrubber Knob */}
                <div
                  className={`absolute top-1/2 w-3 h-3 bg-white rounded-full shadow-lg border-2 border-brand -translate-x-1/2 -translate-y-1/2 transition-opacity duration-200 pointer-events-none ${
                    isDraggingTime
                      ? "opacity-100"
